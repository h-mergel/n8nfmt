...
function checkHardConstraints(
  ...
  const gridBottom = candidateStickies.length > 0 ? Math.max(...candidateStickies.map(s => s.y + s.h)) : 0;
  const gridRight  = candidateStickies.length > 0 ? Math.max(...candidateStickies.map(s => s.x + s.w)) : 0;
  ...
  for (const candNode of candidate.nodes) {
    ...
    if (ny < 0) {
      ...
      if (!declared.includes(candNode.name)) {
        violations.push({ type: 'unexpected_bypass_node', node: candNode.name, expectedSticky: '', severity: 'hard' });
      }
      continue;
    }
    ...
    if (!sticky) {
      violations.push({ type: 'sticky_membership_lost', node: candNode.name, expectedSticky, severity: 'hard' });
      continue;
    }
    if (!(nx >= sticky.x && nx < sticky.x + sticky.w && ny >= sticky.y && ny < sticky.y + sticky.h)) {
      violations.push({ type: 'node_outside_assigned_sticky', node: candNode.name, expectedSticky, severity: 'hard' });
    }
  }
...
function groupViolations(violations: Violation[]): ViolationGroup[] {
  const SUGGESTIONS: Record<RootCause, string> = {
    cross_section_backward_edge: 'source.sectionIdx > target.sectionIdx — add bypass lane (R12) or reroute these nodes',
    intra_section_backward_edge: 'cyclic within section — R5 loop-spreading handles column offset; consider ROW_STEP increase',
    node_outside_section:        'node position falls outside all sticky-note bounding boxes after auto-resize — likely a bypass-lane or orphan node that was not excluded correctly',
  ...
  const touch = (cause: RootCause, v: Violation) => {
    ...
    if ('edge1' in v) { g.edges.add(JSON.stringify(v['edge1'])); g.edges.add(JSON.stringify(v['edge2'])); }
...
// ---------------------------------------------------------------------------
function computeScore(wf: N8nWorkflow): { score: number; rawScore: number; violations: Violation[]; metrics: Record<string, number>; violationGroups: ViolationGroup[] } {
  ...
  for (let i = 0; i < nonSticky.length; i++) {
    for (let j = i + 1; j < nonSticky.length; j++) {
      const a = nonSticky[i]!; const b = nonSticky[j]!;
      const [ax, ay] = a.position; const { width: aw, height: ah } = nodeDimensions(a);
      const [bx, by] = b.position; const { width: bw, height: bh } = nodeDimensions(b);
      if (ax < bx + bw && ax + aw > bx && ay < by + bh && ay + ah > by) {
        nodeOverlaps++;
        violations.push({ type: 'node_overlap', nodes: [a.name, b.name], severity: 'critical', parameterSensitive: true });
      }
    }
  }
  ...
  for (let ei = 0; ei < edges.length; ei++) {
    ...
    for (const node of nonSticky) {
      ...
      for (let pi = 0; pi < path.length - 1 && !hit; pi++) {
        if (segmentIntersectsAABB(path[pi]![0], path[pi]![1], path[pi + 1]![0], path[pi + 1]![1], nx, ny, nx + w, ny + h)) {
          hit = true;
        }
      }
      if (hit) {
        ...
        violations.push({ type: 'edge_node_intersection', edge: [e.from, e.to], node: node.name, severity: 'high', parameterSensitive: et === 'forward', edgeType: et });
      }
    }
  }
  ...
  for (let i = 0; i < edges.length; i++) {
    for (let j = i + 1; j < edges.length; j++) {
      const pA = edgePaths[i]!; const pB = edgePaths[j]!;
      if (pA.length < 2 || pB.length < 2) continue;
      let crossed = false;
      outer: for (let a = 0; a < pA.length - 1 && !crossed; a++) {
        for (let b = 0; b < pB.length - 1; b++) {
          if (segmentsIntersect(pA[a]![0], pA[a]![1], pA[a + 1]![0], pA[a + 1]![1],
                                pB[b]![0], pB[b]![1], pB[b + 1]![0], pB[b + 1]![1])) {
            crossed = true; break outer;
          }
        }
      }
      if (crossed) {
        ...
        violations.push({ type: 'edge_crossing', edge1: [edges[i]!.from, edges[i]!.to], edge2: [edges[j]!.from, edges[j]!.to], severity, parameterSensitive: et === 'forward', edgeType: et });
      }
    }
  }
  ...
  for (const e of edges) {
    ...
    if (src.position[0] > tgt.position[0]) {
      backwardEdges++;
      const et = classifyEdge(e.from, e.to, nodeToSection, nodeMap);
      violations.push({ type: 'backward_edge', edge: [e.from, e.to], severity: 'low', parameterSensitive: false, edgeType: et });
    }
  }
  ...
  for (const e of edges) {
    ...
    if (drift > 0) {
      violations.push({ type: 'y_drift', edge: [e.from, e.to], drift: Math.round(drift), severity: 'low', parameterSensitive: true });
    }
  }
  ...
  for (const node of nonSticky) {
    ...
    const avgTargetY = targets.reduce((s, t) => {
    ...
    if (delta > 20) {
      yAsymmetryCount++;
      violations.push({ type: 'y_asymmetry', node: node.name, delta: Math.round(delta), severity: 'low', parameterSensitive: true });
    }
  }
  ...
  if (stickies.length > 0) {
    ...
    for (const node of nonSticky) {
      const [nx, ny] = node.position;
      const insideAny = stickies.some(s => nx >= s.x && nx < s.x + s.w && ny >= s.y && ny < s.y + s.h);
      if (!insideAny) {
        ...
        nodeOutsideSection++;
        violations.push({ type: 'node_outside_section', node: node.name, severity: 'high', parameterSensitive: true, note: 'sticky note should have been resized by relayout — possible bypass-lane or orphan node' });
      }
    }
  }
...
export function score(workflow: N8nWorkflow, original?: N8nWorkflow): ScoreResult {
  ...
  return {
    ...
    violationGroups:        computed.violationGroups.map(g => ({ rootCause: g.rootCause, count: g.count })),
