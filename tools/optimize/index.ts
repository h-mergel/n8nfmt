  ...
  for (const fixture of files) {
    ...
    try {
      const { workflow: candidateWf } = await relayout(wf, {
        config:  { rowStep: params.ROW_STEP, minGap: params.MIN_GAP, sectionGap: params.SECTION_GAP },
      ...
      process.stderr.write(`  [warn] scoring failed for ${path.basename(fixture)}: ${(e as Error).message.split('\n')[0]}\n`);
    }
  }
...
process.stderr.write(`Baseline score: ${baselineScore.toFixed(1)} (rawScore=${baseline.raw.toFixed(1)})\n`);
...
for (let iter = 1; iter <= MAX_ITER; iter++) {
  ...
  if (noImprovementCount >= 2) { process.stderr.write('Stopping: 2 consecutive no-improvement iterations.\n'); break; }
  if (bestScore >= 90)         { process.stderr.write('Stopping: score >= 90.\n');                             break; }
}
...
try {
  ...
  if (firstFixture !== undefined) {
    ...
    const { workflow: laidOut } = await relayout(wf, {
      config:  { rowStep: bestParams.ROW_STEP, minGap: bestParams.MIN_GAP, sectionGap: bestParams.SECTION_GAP },
...
if (INJECT_BAD_CANDIDATE) {
  ...
  for (const fixture of fixtureFiles) {
    try {
      ...
      if (result.hardConstraintsPassed === false) {
        rejectedByHardConstraints++;
        process.stderr.write(`  [inject-bad-candidate] REJECTED by hard constraints (total rejected: ${rejectedByHardConstraints})\n`);
      } else {
        process.stderr.write(`  [inject-bad-candidate] WARNING: candidate passed hard constraints — not a valid bad candidate\n`);
      }
    } catch (e) {
      process.stderr.write(`  [inject-bad-candidate] error: ${(e as Error).message.split('\n')[0]}\n`);
    }
  }
}
